...
def ReadJson(n: str):
    try:
        with open(
            r"./plugins/database/%s.json" % base64.b64encode(n.encode()).decode(), "r"
        ) as load_f:
...
def CreateJson(n: str, data: dict):
    try:
        with open(
            r"./plugins/database/%s.json" % base64.b64encode(n.encode()).decode(), "w"
        ) as f:
            json.dump(data, f)
        return True
    except:
...
def SaveImg(url: str, n: str, id: str):
    try:
        ...
        urllib.request.urlretrieve(
            url,
            r"./plugins/database/%s%s"
            % (base64.b64encode(n.encode()).decode(), suffix),
        )
